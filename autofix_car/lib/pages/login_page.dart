...
bool _isRegistering = false;
       ...
       try {
         ...
         if (result != null && result.containsKey('idToken')) {
           ...
           ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(content: Text('Welcome, ${_displayEmail}!')),
           );
         ...
         setState(() {
           _authMessage = 'Login Error: ${e.toString().replaceFirst('Exception: ', '')}';
         });
         ScaffoldMessenger.of(context).showSnackBar(
  ...
  child: ElevatedButton(
    onPressed: _isLoggingIn // Disable button while logging in
        ? null
        : _handleLogin,
    style: ElevatedButton.styleFrom(
      backgroundColor: const Color(0xFF3182CE), // Or a different color for login
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
      ),
      elevation: 0,
    ),
    child: _isLoggingIn // Conditional child based on loading state
        ? const SizedBox(
